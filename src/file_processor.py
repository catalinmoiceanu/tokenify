...
class FileProcessor:
    ...
    def process(self, output_path: Path | None) -> None:
        ...
        try:
            ...
            # Error already logged by CommentStripper
            log.error("Skipping file due to tokenization error: %s", self.input_path)
            raise
        except IOError:
            # Error already logged by FileWriter
            log.error(
    ...
    def _read_file(self) -> bytes:
        ...
        try:
            ...
            return content
        except FileNotFoundError:  # Be more specific
            log.error("Failed to read file %s: File not found.", self.input_path)
            raise
        except PermissionError as e:  # Be more specific
            log.error(
