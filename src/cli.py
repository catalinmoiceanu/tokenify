...
class CLIRunner:
    ...
    def setup_and_validate(self) -> bool:
        ...
        # Ensure args is valid before proceeding
        if not self.args:
    ...
    def _setup_logging(self) -> None:
        ...
        # Check if args exist before accessing attributes
        ...
        if not root_logger.hasHandlers():
            ...
            handler.setLevel(log_level)  # Set level on the new handler
        ...
        if output_dir:
            try:
                ...
                if not resolved_output_dir.is_dir():
                    ...
                    sys.exit(1)  # Exit directly on validation failure
    ...
    def _determine_base_path(self) -> None:
        ...
        try:
            ...
            # Fallback: Use parent of the first resolved file
    ...
    def _get_output_destination(
        ...
        if settings.in_place:
            ...
            abs_output_dir = settings.output_dir  # Already resolved
            abs_base_common_path = settings.base_common_path  # Already resolved
            try:
                # Ensure resolved_file_path is absolute before relative_to
            ...
            except OSError as e:  # Catch resolve errors here too
                log.warning(
                    "Could not resolve path for relative calculation '%s': %s",
                    resolved_file_path,
                    e,
                )
                relative_path = Path(resolved_file_path.name)  # Fallback to name
    ...
    def _remove_original_if_compressed_inplace(
        ...
        try:
            # Resolve paths right before comparison/unlink
            ...
            if resolved_original.exists() and resolved_original != resolved_compressed:
                # Use os.unlink which is what Path.unlink uses internally
        ...
        except Exception as e:  # Catch potential resolve errors
            log.warning(
    ...
    def _process_single_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            return False
        except tokenize.TokenError:  # Catch specific error
    ...
    def run(self) -> int:
        ...
        if not self.setup_and_validate():
            # If setup failed (e.g., validation error), exit code should be 1
            # setup_and_validate logs the specific error and might call sys.exit itself
            # but returning 1 ensures a non-zero exit code if it didn't exit.
            return 1
...
Examples (when run as module):
  ...
  python -m src.main -o cleaned_dir -z -a lzma project_dir"""
        ...
        parser = argparse.ArgumentParser(
            ...
            exit_on_error=False,  # Keep False to handle ArgumentError
            epilog=epilog_text,
        )
        parser.add_argument(
            "paths",
    ...
    def run(self, args: Optional[List[str]] = None) -> None:
        ...
        try:
            ...
            sys.exit(2)  # Exit immediately after handling parse error
